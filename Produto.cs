...
public static List<Produto> PesquisarEstoque(string resultadoBusca)
  {
    List<Produto> produtos = new List<Produto>();
    ...
    using (var connection = new SqliteConnection("Data Source=banco.db"))
    {
      ...
      WHERE produto.nome LIKE ($resultadoBusca);";
      command.Parameters.AddWithValue("$resultadoBusca",resultadoBusca);
      using (var reader = command.ExecuteReader())
      {
        while (reader.Read())
        {
